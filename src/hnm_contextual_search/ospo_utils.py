...
logger = logging.getLogger(__name__)
import torch, math, random, regex as re
...
# Download required data (one time)
nltk.download('punkt')
...
def compute_search_owen_rewards(
    ...
    **kwargs
    ...
    for i, (completion_text, phrases) in enumerate(zip(completion_texts, batch_phrases)):
        ...
        target_item = None
        if "target_item_id" in kwargs and i < len(kwargs["target_item_id"]):
            target_item = kwargs["target_item_id"][i]

   
        t_owen = time.perf_counter()
        coalition_rewards = evaluate_search_coalition_rewards(completion_text,
            phrases, coalitions, search_reward_func,
            target_item=target_item, device = device
        )
        owen_ms = (time.perf_counter() - t_owen) * 1000
        search_reward_func.timing_logs["total_call_ms"].append(owen_ms)
        search_reward_func.timing_logs["n_coalitions"].append(len(coalitions))
        # Coalition evaluation for getting marginal contribution of individual phrases. 
        ...
        # print("token_attributions atts", token_attributions)
    ...
    for coalition in coalitions:
        if len(coalition) == 0:
            ...
            if hasattr(search_reward_func, "_evaluate_single_query"):
                reward = search_reward_func._evaluate_single_query(coalition_query, target_item)
            else:
                reward = 0.0  # fallback
            ...
            if sentence_idx not in coalition:
                ...
                if coalition_without in coalition_to_reward and coalition_with in coalition_to_reward:
                    ...
                    if coalition_size < num_sentences:
                        ...
                        # Simplified weighting - just average across coalitions
                        # weight = 1.0 / len(marginal_contributions) if marginal_contributions else 0
